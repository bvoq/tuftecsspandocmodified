...
[//]: <> (Use C-R "   autocmd BufWritePost * !cd ..;make;osascript -e 'on run {curfolder}' -e 'tell application "Safari" to set URL of current tab of front window to "file:///" & curfolder & "/docs/index.html"' -e 'end run' $PWD;)

[//]: <> (tell application "Safari" to do JavaScript "window.location.reload(true)" in current tab in front window)

asdfTufte Pandoc CSS is an attempt to make it as easy as possible to get started
